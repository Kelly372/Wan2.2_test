"""Exchange all DiT self-attention layers at five points of a 25-update run.
...
def attention_trajectory(pipe, scheduler, initial, context, context_null,
    ...
    if swap_index is not None and (swap_index not in selected or donor_snapshots is None):
    ...
    with tempfile.TemporaryDirectory(prefix='attention_cache_', dir=output_dir) as cache_dir:
        with torch.amp.autocast('cuda', dtype=pipe.param_dtype):
            for index, t in enumerate(tqdm(timesteps, desc='Attention experiment')):
                if capture and index in selected:
    ...
    if swap_index is not None and swap_count != 1:
        raise RuntimeError('The trajectory must have exactly one exchange update.')
...
def run_attention_swap(args):
    ...
    output_dir = DATA_DIR / 'video' / args.video_tag
    ...
    with torch.inference_mode():
        ...
        for label in ('A', 'B'):
            ...
            outputs[f'process_{label}_baseline.mp4'] = final
        experiments = []
        for recipient, donor in (('A', 'B'), ('B', 'A')):
            for index, remaining in attention_swap_points():
                filename = f'swap_{recipient}_from_{donor}_step{remaining}.mp4'
                print(f'{filename}: exchange all layers at update {index + 1}/25', flush=True)
                final, _, _ = attention_trajectory(
                    pipe, new_scheduler(), initial[recipient], context, context_null,
                    cfg.sample_guide_scale, output_dir, swap_index=index,
                    donor_snapshots=snapshots[donor])
                outputs[filename] = final
                experiments.append({'file': filename, 'recipient': recipient, 'donor': donor,
                                    'update_1based': index + 1, 'remaining_steps': remaining,
                                    'schedule_index': 25 + index,
                                    'timestep': int(schedule.timesteps[25 + index]),
                                    'sigma': float(schedule.sigmas[25 + index])})
        pipe.model.cpu()
        ...
        for filename, latent in outputs.items():
            ...
            torch.cuda.empty_cache()
    metadata = {
        'experiment': 'all-layer self-attention output exchange',
    ...
    (output_dir / 'attention_swap_metadata.json').write_text(json.dumps(metadata, indent=2), encoding='utf-8')
...
def run_pipeline(model_path, video_tag):
    ...
    print("[2/2] Run A/B baselines and ten all-layer attention exchanges", flush=True)
