...
`--vae_checkpoint` 指向 VAE 权重文件，而非模型目录。`--device cpu` 可使用 CPU；默认以 float32 计算。脚本仅加载 VAE，需要 `torch`、`einops`、`opencv-python` 和 `numpy`，不需要加载扩散模型。

所有数据路径相对于脚本所在的仓库根目录。只需要原视频，首帧视频保持原视频的帧数、分辨率和帧率。

| 输入 | 保存的 latent |
| --- | --- |
| `data/video/clip.mp4`（A） | `data/video/clip.pt` |
| `data/video/clip_first_frame.mp4`（B） | `data/video/clip_first_frame.pt` |
